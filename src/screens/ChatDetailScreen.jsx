...
function PriceCard({ msg, role, onAccept, onDecline, onNegotiate }) {
  ...
  return (
    <div style={{
      background: 'rgba(249,115,22,0.07)', border: `1px solid ${borderColor}`,
      borderRadius: 20, overflow: 'hidden', maxWidth: 280,
    }}>
      <div style={{ padding: '14px 16px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ fontSize: '0.68rem', color: 'rgba(249,115,22,0.7)', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>
          Rental Price
        </div>
      ...
      {isPending && role === 'renter' && !msg.me && (
        <div style={{ padding: '12px 12px 14px', display: 'flex', gap: 8 }}>
          <button onClick={onDecline} style={{ flex: 1, padding: '10px', borderRadius: 100, border: 'none', background: 'rgba(239,68,68,0.12)', color: '#ef4444', fontFamily: 'inherit', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' }}>
            Decline
          </button>
          <button onClick={onNegotiate} style={{ flex: 1, padding: '10px', borderRadius: 100, border: 'none', background: 'rgba(249,115,22,0.15)', color: '#f97316', fontFamily: 'inherit', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' }}>
            Negotiate
          </button>
          <button onClick={onAccept} style={{ flex: 1, padding: '10px', borderRadius: 100, border: 'none', background: '#22c55e', color: '#fff', fontFamily: 'inherit', fontSize: '0.82rem', fontWeight: 700, cursor: 'pointer' }}>
            Accept
          </button>
        </div>
      )}
  ...
  if (msg.type === 'price_offer') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: msg.me ? 'flex-end' : 'flex-start', marginBottom: 8 }}>
        {!msg.me && (
          <div style={{ width: 24, height: 24, borderRadius: '50%', marginBottom: 4, background: chat.car.colorBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem', fontWeight: 700, color: chat.otherColor }}>
            {chat.otherInit}
          </div>
        )}
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      <div style={{ display: 'flex', flexDirection: msg.me ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: 6 }}>
        {!msg.me && (
          <div style={{ width: 24, height: 24, borderRadius: '50%', flexShrink: 0, background: chat.car.colorBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem', fontWeight: 700, color: chat.otherColor }}>
            {chat.otherInit}
          </div>
        )}
        <div style={{
...
export default function ChatDetailScreen({ chat, role = 'renter', onBack }) {
  ...
  // Simulate the other party typing and replying
  function simulateReply(replyText, delay = 800) {
    ...
    setTimeout(() => {
      ...
      setMessages(prev => [...prev, {
        ...
        text: replyText, me: false, time: new Date(),
      }])
    }, delay)
  }
  ...
  function handleAccept(msgId) {
    ...
    simulateReply("Deal confirmed! I will prepare the car. See you soon 🚗", 700)
  }

  function handleDecline(msgId) {
    updateOfferStatus(msgId, 'declined')
    simulateReply('No worries — feel free to send a counter offer anytime.', 600)
  }

  function handleNegotiate(msgId) {
    updateOfferStatus(msgId, 'negotiating')
    setMessages(prev => [...prev, { id: Date.now().toString(), type: 'text', text: "I'd like to negotiate the price.", me: true, time: new Date() }])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: '#0D0D0D' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px', background: '#111', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ width: 34, height: 34, borderRadius: 10, flexShrink: 0, background: chat.car.colorBg, overflow: 'hidden', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem' }}>
          {chat.car.photo
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: '0.7rem', color: isTyping ? '#f97316' : 'rgba(255,255,255,0.4)', transition: 'color .2s' }}>
            {isTyping ? `${chat.otherName} is typing…` : chat.otherName}
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        {messages.map(msg => (
          <MessageRow
            key={msg.id} msg={msg} role={role} chat={chat}
        ...
        {isTyping && <TypingIndicator chat={chat} />}
        <div ref={bottomRef} />
      </div>
